...
public class ForgotPasswordTests {
    ...
    public void forgotPassword_Success() {
        ...
        boolean found = false;
        String token = "";
        for (int i = 0; i < sendForgotPasswordEmailResponse.getUsercodeList().size(); i++) {
            if (USERCODE.equals(sendForgotPasswordEmailResponse.getUsercodeList().get(i))) {
                found = true;
        ...
        resetPasswordRequest.setPassword(USERNAME);
        resetPasswordRequest.setPasswordRetype(USERNAME);
    ...
    public void createUser_Success() {
        ...
        String middleName = "";
        try {
            List<User> userList = UserDAO.getInstance().findByUsername(username);
            if (userList.size() == 0) {
                User user = new User();
