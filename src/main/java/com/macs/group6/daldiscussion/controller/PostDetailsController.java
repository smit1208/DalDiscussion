...
public class PostDetailsController {
    ...
    public String viewPostDetails(ModelMap model, @PathVariable("id") int post_id, HttpSession session) {
        ...
        try {
            images = iServiceFactory.createImageService().getImageByPostId(post_id);
        } catch (DAOException e) {
            logger.error(e.getMessage());
            return "customError";
        }
        try {
            commentMap = iServiceFactory.createCommentService().getComments(post_id);
        } catch (DAOException e) {
            logger.error(e.getMessage());
            return "customError";
        }
        ...
        for (int i=0; i<commentList.size();i++) {
            try {
                replyList = iServiceFactory.createReplyService().getReplies(commentList.get(i).getId());
            } catch (DAOException e) {
                logger.error(e.getMessage());
                return "customError";
            }
    ...
    public String addComment(@RequestParam("comment") String comment, ModelMap model, @PathVariable("id") int post_id, HttpSession session) {
        ...
        try {
            iServiceFactory.createCommentService().addComment(c, post_id,user_id,name);
        } catch (DAOException e) {
            logger.error(e.getMessage());
            return "customError";
        }
    ...
    public String addReply(@RequestParam("reply") String reply, ModelMap model, @PathVariable("id") int post_id, @PathVariable("c_id") int comment_id,
        ...
        try {
            iServiceFactory.createReplyService().addReply(replies, comment_id,user_id,name);
        } catch (DAOException e) {
            logger.error(e.getMessage());
            return "customError";
        }
